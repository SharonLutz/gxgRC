...
##############################################################
 mat_results <- matrix(0,nrow=length(betaI),ncol=3)
  colnames(mat_results) <- c("model0:X1","model1:X1","model2:X1X2")
...
##############################################################
      ...
      if(summary(model2)$coefficients[4,4] < alpha_level){mat_results[bi,"model2:X1X2"] <- mat_results[bi,"model2:X1X2"] +1 }
##############################################################
# Fit model 0: Just X1
##############################################################
model0 <- lm(Y~X1) 
...
##############################################################
  if(plot.pdf){
    ...
    legend("topleft",lty=c(2:4),col=c(1:3),pch=c(1:3),legend=c("model0:Y~X1","model1:Y~X1+X2","model2:Y~X1+X2+X1*X2"))
    dev.off()
  }
